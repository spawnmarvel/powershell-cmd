...
Write-Host "# V1.0"
Write-Host "# Set env before install"
Write-Host "# Install RabbitMQ from local dir (must be in same folder as script)"
...
if ($null -eq $rmq_base_home) 
{
   ...
   Write-Host "Env is present: " $rmq_base " : " $rmq_base_home
...
if ($null -eq $rmq_conf_home) 
{
   ...
   Write-Host "Env is present: " $rmq_conf " : " $rmq_conf_home
}
...
if ($null -eq $rmq_conf_adv_home) 
{
   ...
   Write-Host "Env is present: " $rmq_conf_adv " : " $rmq_conf_adv_home
